...
% run this in command: CreateCircos([1, 50], 0.08); 
% Readme.md will explain the 
...
for Counter = 1:length(NEWPOWERNAME)
    ChrStr = ['chr - hs', num2str(Counter), ' ', char(NEWPOWERNAME(Counter)), ' ', num2str(min(Bands)), ' ', num2str(max(Bands)+1), ' chr22']; % was chr22
    ...
    for DataCounter = 1:length(AllData(:,1))
        if isempty (strfind (AllNames{DataCounter}, 'XXX')) && ~isempty (strfind (AllNames{DataCounter}, NEWPOWERNAME{Counter})) 
            ...
            Freq = round((str2num (DataName(idx(end)+1:end)))./20); %added ./20 to divide the freq by 20

            if AllData (DataCounter,1) >=Rho_Thresh && Freq>=min(Bands) && Freq<=max(Bands) 
              Freq = Freq;
                ColorStr = ['hue', sprintf('%03d' ,round(150*((Freq-1)/max(Bands)) + 175))]; %change 175 value for different colour scheme. default: 175
                % ColorStr = ['dpblue'];
...
for DataCounter = 1:length(AllData)
    if ~isempty (strfind (AllNames{DataCounter}, 'XXX')) 
        DataName = AllNames{DataCounter};
        idx = strfind(DataName, '_');
        Freq = round((str2num (DataName(idx(end)+1:end)))./20);
        %Freq = str2num(DataName(idx(end)+1:end));
